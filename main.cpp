...
void bubblesort(unsigned short arr[], unsigned short n)
{
    ...
    bool verif = true;
    while (verif == true)
    {                  // as long as a swap has happened during a loop, the same loop will repeat itself
...
{                                                  // This function takes the last element as pivot, places the pivot element at its correct position in sorted array, and places all the smaller elements than the pivot to the left of it and all the greater elements to the right of it
