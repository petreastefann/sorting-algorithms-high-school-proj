...
int partition(int arr[], int firstindex, int lastindex) {  // This function takes the last element as pivot, places the pivot element at its correct position in sorted array, and places all the smaller elements than the pivot to the left of it and all the greater elements to the right of it
    ...
    std::swap(arr[i], arr[lastindex]);  // its arr[lastindex] instead of pivot because the function swaps the elements on the indexes ("pointed" by the indexes).std::swap() looks at the indexes not at the elements themselves
    return (i);
}
...
void bogosort(std::vector<int>& arr) {
    ...
    std::cout<<"s-au facut "<<iterations<<" iteratii\n";
}
        ...
        switch (m) {
            ...
            case '6': {
                ...
                if (bgs_m == 0) {
                    ...
                    std::vector<int> arr = {3, 2, 5, 1, 7, 6, 4};
                    std::cout << "pentru sirul de numere:";
